...
const ProfileSection = ({ username, avatar }) => {
  ...
  const fetchProfileAndNetwork = async () => {
    ...
    try {
      ...
      const pendingRes = await fetch(`${API_BASE_URL}/api/connections/pending`, { headers });
      if (pendingRes.ok) {
      ...
      const listRes = await fetch(`${API_BASE_URL}/api/connections/list`, { headers });
      if (listRes.ok) {
      ...
      if (res.ok) {
        ...
        if (stored) {
          try {
            ...
            localStorage.setItem('ucw_current_user', JSON.stringify({ ...parsed, ...updatedUser }));
          } catch (err) {}
        }
  ...
  const handleReject = async (connectionId) => {
    try {
      const token = localStorage.getItem('ucw_access_token');
      const res = await fetch(`${API_BASE_URL}/api/connections/reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
    ...
    <div className="w-full h-full p-4 overflow-y-auto pb-24 custom-scrollbar">
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <button
            onClick={() => {
              localStorage.removeItem('ucw_access_token');
